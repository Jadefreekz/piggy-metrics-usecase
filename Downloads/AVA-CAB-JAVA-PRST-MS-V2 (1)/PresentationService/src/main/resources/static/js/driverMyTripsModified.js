...
function prefillTripDetails(viewTripDetails) {
	...
	for (let i = 0; i < bookingRequests.length; i++) {
		...
		//Employee Reached and trip Onprogress
		if (bookingRequests[i].status == "Reached" && tripDetails.requestType == "Pickup" && tripDetails.status == "Onprogress") {
...
function pickupIconClicked(bookingId, id) {
	...
	xhr.onreadystatechange = function() {
		...
		}else if(xhr.readyState == 4 && xhr.status == 881){
			window.location.reload();
		}
	};
	xhr.send();
