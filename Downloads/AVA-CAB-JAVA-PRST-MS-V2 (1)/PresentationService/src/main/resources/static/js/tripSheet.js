...
try{
	...
	if (xhr.readyState == 4 && xhr.status == 200) {
		...
		if(request == "Drop"){
			...
			window.location.href = "/admin/ongoingtrip-pickup?tripCabId=" + tripCabId;
		}	
		}
		...
		if (request == "Drop") {
			...
			if (showCount > 0) {
				if (noShowListBookingId == '' && noShowListEmpId == '') {
					updateEmpStatus = createHttpRequest("PUT", pathName + "/bookingInfoService/tripService/updateTrip/" + tripCabId + "/" + 0 + "/" + 0 + "/" + false+"?requestType="+request, true, "ADMIN");
					...
					updateEmpStatus = createHttpRequest("PUT", pathName + "/bookingInfoService/tripService/updateTrip/" + tripCabId + "/" + noShowListEmpId + "/" + noShowListBookingId + "/" + false+"?requestType="+request, true, "ADMIN");
					updateEmpStatus.setRequestHeader("Content-Type", "application/json");
					updateEmpStatus.send();
...
//		var dropPoint = document.getElementById("Droppoint4" + rowId);
		var empLocation = document.getElementById("hideLocationTextBox");
		...
		if(request == 'Pickup'){
			var editLocation = document.getElementById("tr"+rowId).getElementsByTagName('td')[6].innerHTML;
			bookingId = document.getElementById("tr" + rowId).getElementsByTagName('td')[8].innerHTML;
		}else{
...
//		dropPoint.value = editDrop;
		if(request == 'Pickup'){
			empLocation.value = editLocation;
		}
...
try{
		...
		if(request == 'Drop'){
			...
			bookId = document.getElementById("tr" + rowId).getElementsByTagName('td')[8].innerHTML;
		}
